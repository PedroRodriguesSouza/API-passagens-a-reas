...
data class RetrieveTicket(
    ...
    private fun getSuccessIntegrations(integrationsResponse: List<IntegrationOutput>): List<Ticket>{
        return integrationsResponse.asSequence().filterIsInstance<IntegrationSuccess>().map{
            it.data.toEntity()
        }.toList()
    }
    ...
    private fun createOutput(input: RetrieveTicketInput, lowestTicketPrice: Ticket?): RetrieveTicketOutput {
        if(lowestTicketPrice == null){
            return RetrieveTicketError(message = "Could not extract tickets for ${input.origin} to ${input.destination}")
        }
        else
            return RetrieveTicketSuccess(ticket = lowestTicketPrice)
    }
}